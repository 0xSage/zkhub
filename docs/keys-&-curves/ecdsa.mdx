...
## Technical Brief
Given a ECDSA keypair of: 
- Private key: $k_{priv}$
- Public key: $k_{pub}$, from ${\displaystyle k_{pub}=k_{priv}\times G}$

Over an appropriate $({\textrm {CURVE}},G,n)$ where $G$ is a generator point on the curve, and $n$ is the multipliative order of the point G. 
### Signature Generation
A message $m$ is signed as follows: 
1. ${\displaystyle h={\textrm {HASH}}(m)}$. See [hashes](/docs/primitives/hashes)
2. Calculate a point: $(x_{1},y_{1})=k\times G$ where $k$ is a random number in ${\displaystyle [1,n-1]}$
3. Calculate $r=x_{1}\,{\bmod {\,}}n$
4. Calculate ${\displaystyle s=k^{-1}(z+rk_{priv})\,{\bmod {\,}}n}$, where $z$ is the leftmost bits of $h$ up to the bitlength of the group order $n$
...
3. Calculate ${\displaystyle u_{1}=zs^{-1}\,{\bmod {\,}}n}$ and ${\displaystyle u_{2}=rs^{-1}\,{\bmod {\,}}n}$.
